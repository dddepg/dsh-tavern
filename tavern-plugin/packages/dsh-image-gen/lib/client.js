...
.dsh-ig-workbench-grid{display:grid;grid-template-columns:minmax(190px,238px) minmax(0,1fr) minmax(286px,340px);grid-template-rows:minmax(0,1fr);min-height:0;height:0;flex:1 1 0;border:1px solid var(--ig-line);border-radius:12px;overflow:hidden;background:var(--background,#fff);box-shadow:0 12px 32px rgba(30,42,76,.05)}
...
.dsh-ig-collapse-btn{width:28px;height:28px;display:inline-flex;align-items:center;justify-content:center;border:1px solid transparent;border-radius:6px;background:transparent;color:#717b90;cursor:pointer;transition:background .15s,color .15s,border-color .15s}
...
.dsh-ig-generate-button{height:42px;flex:0 0 auto;border:0;border-radius:8px;background:linear-gradient(135deg,#386cf6,#2758e7);color:#fff!important;display:flex;align-items:center;justify-content:center;gap:8px;font-size:12px;font-weight:650;cursor:pointer;box-shadow:0 8px 18px rgba(47,100,245,.22)}.dsh-ig-generate-button:disabled{opacity:.55;cursor:not-allowed;box-shadow:none}
...
.dsh-ig-gallery-card{background:var(--dsw-alias-bg-layer-2,#ffffff);border:1px solid var(--dsw-alias-border-l2,#e2e8f0);border-radius:10px;overflow:hidden;display:flex;flex-direction:column;cursor:pointer;transition:border-color .18s ease}
@media (hover:hover) and (pointer:fine){.dsh-ig-gallery-card:hover{border-color:var(--dsw-alias-border-l1,#cbd5e1)}}
...
.dsh-ig-gallery-card-img{width:100%;height:100%;object-fit:cover}
...
.dsh-ig-gallery-page-toast{position:fixed;bottom:24px;left:50%;opacity:1;transform:translate(-50%,0);background:rgba(0,0,0,0.85);color:#fff;padding:6px 14px;border-radius:8px;font-size:13px;z-index:99999;animation:dshIgPageToast 150ms cubic-bezier(0.23,1,0.32,1)}
@keyframes dshIgPageToast{from{opacity:0;transform:translate(-50%,6px)}}
@media (prefers-reduced-motion:reduce){.dsh-ig-gallery-page-toast{animation:none}}
