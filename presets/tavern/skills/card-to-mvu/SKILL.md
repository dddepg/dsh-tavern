...
保存成功、自动检查通过且删除清单符合任务后，直接交付。成品开场保留 `<initvar>…</initvar>` 供 MVU 初始化，并保留 `<mvu-status/>` 供显示正则生成面板；它们由工具安装，属于预期结构，不是旧内容残留。不要因为搜索命中这些标记而删除、重新转换或继续调查。实际界面泄漏标记或 JSON 才属于展示故障，应另行诊断。交付报告包含副本路径、实际修改和自动校验结果，即可宣布转换完成。真实模型游玩、浏览器布局检查和剧情节奏调优仅在用户明确要求时另行执行，不作为改卡完成条件，也不默认列成“待验收”清单。limitations（旧版工具为 pending）仅说明自动检查的覆盖范围；需要说明时用一句“自动校验通过，未进行真实游玩测试”，不要求用户补做验收，不把未实测说成已通过。仅编辑本 Skill 不触发人物卡转换或模型游玩。
